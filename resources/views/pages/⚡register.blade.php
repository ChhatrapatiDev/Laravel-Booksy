...
new class extends Component
{
    ...
    public function register()
    {
        ...
        // Automatically hash the password (Laravel 10+ does this automatically via model casts, but explicit is safe)
        $validated['password'] = Hash::make($validated['password']);

        $user = User::create($validated);

        // Best practice: Log the user in right after registering
        Auth::login($user);
        session()->regenerate();
...
<div class="w-full max-w-md">
    <div class="text-center mb-8">
        ...
        <p class="mt-2 text-sm text-slate-500 dark:text-slate-400">Get started with your free account today.</p>
    </div>
    ...
    <form wire:submit="register"
        ...
        <div>
            ...
            @error('name') <span class="block mt-1.5 text-xs font-medium text-rose-500 dark:text-rose-400">{{ $message
                }}</span> @enderror
        </div>
        ...
        <div>
            <label
                class="block text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2">Password</label>
        ...
        <div>
            <label
                class="block text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2">Confirm
                Password</label>
